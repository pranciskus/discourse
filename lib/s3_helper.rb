...
class S3Helper
  ...
  def self.build_from_config(use_db_s3_config: false, for_backup: false, s3_client: nil)
    ...
    role_session_name = setting_klass.s3_role_session_name.presence
  ...
  def self.s3_options(obj, role_session_name: nil)
    ...
    # Try role assumption if fully configured
    if role_session_name.present?
      role_arn = obj.s3_role_arn.presence
    ...
    # Fall back to explicit keys if no credentials set yet
    if opts[:credentials].nil? && obj.s3_access_key_id.present? && obj.s3_secret_access_key.present?
